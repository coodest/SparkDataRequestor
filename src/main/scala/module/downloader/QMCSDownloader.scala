...
class QMCSDownloader(var context: Context) extends Downloader with Runnable {
  ...
  private def sampleMiu(segment: Array[Integer], sampleRatio: Double): Double = {
    ...
    var sampleSequence: Array[Int] = null
    if (segment(0) eq segment(1)) {
      sampleSequence = new Array[Int](1)
      sampleSequence(0) = segment(0)
    }
    else {
      val bases: Array[Int] = Array(2)
      sampleSequence = new Array[Int](sampleNum)
      val temp: ListBuffer[Array[Double]] = sg.haltonSeq(sampleNum, bases)
      var i: Int = 0
      while (i < sampleNum) {
        sampleSequence(i) = (temp(i)(0) * (Math.abs(segment(1) - segment(0)) + 1) + segment(0)).toInt
        if (i > 0 && sampleSequence(i - 1) == sampleSequence(i)) {
          sampleSequence(i) += 1
        }
